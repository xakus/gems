...
- **Фаза 2 (рабочие показатели)** — карточки с live-графиком (`fl_chart`); появляются по мере прихода данных с анимацией «от маленького к большому». **Напряжение и ток — трёхфазные** (3 линии A/B/C), мощность/обороты/нагрев/частота — однофазные. На каждом графике — **горизонтальная линия заданного номинала** (из введённых параметров; у нагрева номинала нет), а в карточке — текущее значение и «задано». Раскладка адаптивная (заполняет экран без пустот, до 3 колонок на 1920×1080, прокрутка на узких экранах). Колонка `phase` в `test_measurements` хранит фазу (1/2/3, либо 0).
