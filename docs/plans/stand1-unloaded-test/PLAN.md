...
## 12.1. Доработка 1 (трёхфазность + номиналы + адаптив) — выполнено

Запрошено заказчиком после первой реализации:
- [x] Напряжение и ток — **по 3 фазам** (A/B/C, 3 линии на графике, лёгкий дисбаланс в моке).
- [x] Колонка `phase` в `test_measurements` (БД **v5**, миграция `v4→v5`); модель/мок/контроллер обновлены.
- [x] Частота — отдельной карточкой с живым графиком.
- [x] **Заданные перед стартом значения** показаны в каждой карточке + **горизонтальная линия номинала** на графике (нагрев — без линии).
- [x] Адаптивная заполняющая сетка под **1920×1080 / 32″** (`LayoutBuilder`, до 3 колонок, без пустот, прокрутка на узких), крупные шрифты, `FittedBox` от переполнения.
- [x] Локализация `metric_frequency`, `test_nominal_label` (en/az/ru); доки обновлены.

## 12.2. Доработка 2 (по замечаниям при прогоне) — выполнено

- [x] Подписи фаз **U/V/W** вместо A/B/C (`kPhaseLabels` в config).
- [x] **Сглаживание графиков**: стабильная ось Y (привязка к номиналу), скользящее окно `kChartWindowPoints`, `curveSmoothness`/`preventCurveOverShooting`, мягкое появление карточек (easeOutCubic вместо отскока).
- [x] **Фаза 1 информативнее**: удлинена (`kMockPhase1DurationMs` 3.5с→9с), добавлены прогресс-бар и чеклист проверок (межвитковое/обрыв/пробой ВН/КЗ на корпус) с поочерёдными галочками; крупные шрифты. Ключи `test_check_*`.

## 12.3. Доработка 3 (синхронный hover графиков) — выполнено

- [x] **Crosshair как в Grafana**: общий `ValueNotifier<int?> _hover` на экране, проброшен во все `MetricCard`. Наведение мышью на любой график → вертикальная линия и значения показываются **синхронно во всех карточках** на той же позиции по времени.
- [x] Данные выровнены по правому краю общей X-шкалы (`kChartWindowPoints`), значения под курсором берутся по индексу `_indexAt`; при отсутствии данных — текущие.
