...
- **`test_runs`** — запуски тестов двигателей: стенд, режим, параметры двигателя, `status` (`running`/`passed`/`aborted`/`failed_*`), `error_code`, итоги омметров, кто/когда запустил.
- **`test_measurements`** — тайм-серия измерений (FK → test_runs, `ON DELETE CASCADE`): метрика, **`phase`** (1/2/3 для трёхфазных напряжения/тока, 0 — однофазные), значение, единица, время. Для графиков и отчётов.
- **`test_events`** — журнал событий теста (FK → test_runs, CASCADE): старт, успех фазы 1, ошибка, экстренный стоп, финиш.

История миграций: v1→v2 добавила мягкое удаление + audit_log; v2→v3 добавила compressor_templates; v3→v4 добавила test_runs/test_measurements/test_events (модуль тестирования); v4→v5 добавила колонку `phase` в test_measurements (трёхфазность).
...
Поток: карточка стенда → экран выбора режима (`stand_N_screen.dart`) → выбор **«С нагрузкой»** (`/stand-N/loaded` → `StandTestScreen`) или **«Без нагрузки»** (`/stand-N/unloaded` → `MotorParamsUnloadedScreen` → форма параметров → кнопка **«Старт»** → `/stand/unloaded/test` → `StandTestUnloadedScreen`). Стенд 5 (компрессор) ведёт на `CompressorParamsScreen` с шаблонами из `compressor_templates`.

### Окно теста «Без нагрузки» (`features/stands/test/`) — реализовано

`StandTestUnloadedScreen` — процесс теста двигателя. Получает `MotorParams` + `standId` через `arguments` маршрута. Архитектура:
- **`PlcDataSource`** (`data/services/plc/`) — абстракция источника данных ПЛС; пока реальной ПЛС нет, работает **`MockPlcDataSource`** (таймеры: омметры приходят раньше, рабочие метрики — параллельно по времени). Реальную ПЛС подключают новой реализацией интерфейса, не трогая UI.
- **`TestController`** (`ChangeNotifier`, локально на экране) — подписан на ПЛС, агрегирует состояние, пишет запуск/измерения/события в **`TestRepository`**.
- **Фаза 1 (омметры)** в центре с анимацией; при успехе уезжает наверх строками с ✓; при аварии (КЗ межвитковое/обрыв/пробой по ВН/КЗ на корпус) — `TestErrorOverlay` по центру, статус `failed_*` в БД.
- **Фаза 2** — карточки `MetricCard` с live-графиком (**`fl_chart`**), появляются по приходу данных с анимацией «от маленького к большому». **Напряжение и ток — трёхфазные** (3 линии A/B/C, цвета `kPhaseColors`); мощность/обороты/нагрев/частота — однофазные. На графиках — **горизонтальная линия заданного номинала** (из `MotorParams`; у нагрева номинала нет). Каждая карточка показывает текущее значение и «задано». Раскладка — адаптивная заполняющая сетка (`LayoutBuilder`, до 3 колонок на 1920×1080, прокрутка на узких).
- Пока тест идёт — выход назад заблокирован (`PopScope`). Есть кнопка **экстренного завершения** (статус `aborted`).

`StandTestScreen` (режим «С нагрузкой») пока заглушка. PDF-экспорт результатов планируется через `pdf` + `printing` (уже в зависимостях), данные берутся из `test_measurements`.
