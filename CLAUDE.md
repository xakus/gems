...
- **`test_measurements`** — тайм-серия измерений (FK → test_runs, `ON DELETE CASCADE`): метрика, значение, единица, время. Для графиков и отчётов.
- **`test_events`** — журнал событий теста (FK → test_runs, CASCADE): старт, успех фазы 1, ошибка, экстренный стоп, финиш.

История миграций: v1→v2 добавила мягкое удаление + audit_log; v2→v3 добавила compressor_templates; v3→v4 добавила test_runs/test_measurements/test_events (модуль тестирования).
...
- **Фаза 2** — карточки `MetricCard` (напряжение/ток/мощность/обороты/нагрев) с live-графиком (**`fl_chart`**), появляются по приходу данных с анимацией «от маленького к большому».
